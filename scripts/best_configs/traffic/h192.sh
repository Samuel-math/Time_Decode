...
out="${TD_FINETUNE_ROOT:-search_traffic_192_noexpert_fixed96_r4_lr5e4}"
mkdir -p "$out"
...
python -u decoder_only_NTP/patch_vqvae_finetune.py --dset traffic \
  --context_points 96 --target_points 192 --batch_size 16 --num_workers 0 \
  --scaler standard --features M --channel_indices "$channels" --channel_group_id 0 \
  --pretrained_model "$pre" --n_epochs 50 --lr 5e-4 --weight_decay 1e-4 \
  ...
  --ar_step_size 12 --pred_len 14 --use_group_channel_experts 0 \
