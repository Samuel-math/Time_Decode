...
if [ "${TD_STAGE:-}" != finetune ]; then
  echo "Historical best from-scratch provenance is incomplete for traffic." >&2
  echo "For checkpoint-based fine-tuning only: TD_STAGE=finetune HORIZONS=96 bash scripts/traffic_best.sh" >&2
  exit 2
fi
...
for horizon in $HORIZONS; do
  bash "$SCRIPT_DIR/best_configs/traffic/h$horizon.sh"
done
